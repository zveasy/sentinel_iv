...
from hb.adapters import cmapss_fd003  # noqa: E402
...
def main():
    parser = argparse.ArgumentParser(description="CMAPSS FD003 regression harness")
    parser.add_argument(
        "--manifest",
    ...
    parser.add_argument(
        "--thresholds",
        default=os.path.join(ROOT_DIR, "configs", "cmapss_fd003_thresholds.yaml"),
    )
    ...
    spike_guard = thresholds.get("spike_guard") or {}
    spike_enabled = bool(spike_guard.get("enabled", False))
    spike_percentile = spike_guard.get("severe_threshold_percentile")
    spike_persistence_cycles = spike_guard.get("severe_persistence_cycles")
    if spike_percentile is not None:
        spike_percentile = float(spike_percentile) / 100.0
    if spike_persistence_cycles is not None:
        spike_persistence_cycles = int(spike_persistence_cycles)

    if thresholds.get("variant") == "fd003" and not spike_enabled:
        print("FAIL fd003 requires spike_guard.enabled == true", file=sys.stderr)
        return 1
    ...
    train_file = os.path.join(root, dataset.get("files", {}).get("train", "train_FD003.txt"))
    ...
    for window in windows:
        ...
        try:
            raw = cmapss_fd003.parse(temp_path)
    ...
    for window in windows:
        ...
        if window["name"] == baseline_name:
            ...
            scores = [
                _cycle_score(row, baseline_mean, baseline_std) for row in current_rows
            ]
            ...
            if spike_enabled and spike_persistence_cycles and expected_fail:
                spike_threshold = None
                if spike_percentile is not None:
                    spike_threshold = _percentile(scores, spike_percentile)
                if spike_threshold is None:
                    spike_threshold = fail_threshold
                max_streak = 0
                current = 0
                for score in scores:
                    if score >= spike_threshold:
                        ...
                        current = 0
                if max_streak >= spike_persistence_cycles:
                    status = "FAIL"
