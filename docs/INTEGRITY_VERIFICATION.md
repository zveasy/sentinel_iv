...
If you have a `.sha256` file next to the zip, run the command **from the directory that contains both the zip and the .sha256 file** (e.g. your download folder or `artifacts/`):

**Linux / macOS:**
```bash
cd /path/to/directory/containing/zip/and/sha256
sha256sum -c hb-hybrid-kit-vX.Y.Z.zip.sha256
# or on macOS:
shasum -a 256 -c hb-hybrid-kit-vX.Y.Z.zip.sha256
```

Example from this repo after building: `cd artifacts && shasum -a 256 -c hb-hybrid-kit-v0.3.0.zip.sha256`
